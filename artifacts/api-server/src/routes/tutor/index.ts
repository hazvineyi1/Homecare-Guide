...
export function buildSystemPrompt(topicId: number, level: string, learnerName = "", country = "", scenario = ""): string {
  ...
  const localeNote = country
    ? `\n\nLEARNER LOCALE: The learner is in ${country}. Set EVERY scenario and example in ${country}. Use first names, family and relationship terms, neighbourhood, village or town types, foods, transport, money and currency, health-system terms, and everyday situations that are authentic and immediately recognisable to people who live there. Reflect the country's diversity naturally. Crucially, keep the clinical guidance from the chapter unchanged: localise only the human and cultural details, never the medical facts or safety advice.`
    : "";

  const skinSignHelp =
    "When you describe or teach skin signs (redness or inflammation, paleness, a blue or grey tinge / cyanosis, jaundice, bruising, pressure areas), describe them the way they actually appear on the skin tone of the person in the scenario. On darker skin, check the lips, gums, tongue, palms and soles, compare against the person's own normal skin tone, and use warmth, firmness and shine as well as colour rather than assuming light-skinned appearances. Keep this consistent with the chapter's clinical facts.";
  const worldviewNote = country
    ? `\n\nWORLDVIEW AND SKIN: Reflect the people, names, family and community life, and the range of skin tones that are actually common in ${country}, including its real diversity. ${skinSignHelp}`
    : `\n\nDEFAULT WORLDVIEW: With no country chosen, present an African and Black worldview by default (the book's own context), with African names, family and community life, and darker skin tones. ${skinSignHelp}`;
...
16. End every turn with exactly ONE clear question that moves the learner forward, except the [SYNTHESIS] turn, which ends by inviting them to continue. Your reply is always a response to the learner within the flow of that question.${localeNote}${worldviewNote}${adaptNote}${nameNote}`;
}
